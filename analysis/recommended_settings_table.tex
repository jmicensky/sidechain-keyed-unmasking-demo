...
%
% The "Engaged" column (fraction of the clip crossing threshold) was
% dropped from the table itself to save width/help it fit the page -
% the three engagement percentages (68.8%, 29.9%, 1.9%) are kept in the
% footnote below instead, since that tradeoff is still the main point of
% presenting three tiers rather than one "optimal" row. \scriptsize (not
% \footnotesize, unlike the sibling tables) for extra width margin, since
% this table still has 8 columns plus text tier labels wider than a
% numeric rank column.
...
identified in the 150-point parameter sweep. Gains are relative to the
unprocessed baseline.}
\label{tab:recommended-settings}
\begin{tabular}{@{}l S[table-format=-2.1] S[table-format=1.1] S[table-format=2.1] S[table-format=2.1] S[table-format=3.0] S[table-format=+1.2] S[table-format=+1.3] S[table-format=+2.1]@{}}
\toprule
\cellcolor{paramgrey}\textbf{Tier} & \cellcolor{paramgrey}{\textbf{Thr.} (\si{\decibel})} & \cellcolor{paramgrey}{\textbf{Ratio}} & \cellcolor{paramgrey}{\textbf{Knee} (\si{\decibel})} & \cellcolor{paramgrey}{\textbf{Atk.} (\si{\milli\second})} & \cellcolor{paramgrey}{\textbf{Rel.} (\si{\milli\second})} & \textbf{Margin Gain} (\si{\decibel}) & \textbf{STOI Gain} & \textbf{Loud.\ Red.\ (pp)} \\
\midrule
Aggressive   & -58.6 & 7.4 & 5.9  & 3.2  & 215 & \cellcolor{green!70}+6.96 & \cellcolor{yellow!70}+0.104 & \cellcolor{orange!70}+11.4 \\
Balanced     & -40.3 & 5.7 & 22.9 & 16.9 & 347 & \cellcolor{green!60}+6.21 & \cellcolor{yellow!67}+0.100 & \cellcolor{orange!60}+9.8  \\
Conservative & -28.8 & 7.7 & 9.4  & 8.5  & 384 & \cellcolor{green!23}+2.40 & \cellcolor{yellow!27}+0.042 & \cellcolor{orange!25}+3.0  \\
\bottomrule
\end{tabular}
\par\vspace{4pt}
...
was not ruled out, only untested. Each tier's threshold engages the
sidechain (dry Dialogue envelope exceeding threshold) for a different
share of the clip: Aggressive \SI{68.8}{\percent}, Balanced
\SI{29.9}{\percent}, Conservative \SI{1.9}{\percent} -- the metric gains
above come at the cost of the compressor being active that much of the
time, directly opposing transparency when Dialogue is silent.
\end{table}
